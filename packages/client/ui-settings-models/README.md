...
The page keeps a route's model list current without a manual refresh. When the **auto-sync** toggle is on, after each load it force-live interrogates every configured pi-ai route's endpoint (`forceLive`) and writes the models that endpoint advertises **add-only** into that route's `models` list. A model a route already serves is kept verbatim, so user-tuned entries survive; a route with nothing new is left untouched. The toggle is off by default because the action asks each endpoint over the wire on every page open, and the choice is stored in `dsh.modelsSettings.autoSync`.
