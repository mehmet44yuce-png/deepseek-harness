...
本页面无需手动刷新即可让某路由的模型列表保持最新。当**自动同步**开启时，每次加载后都会对每条已配置的 pi-ai 路由的端点执行 `forceLive` 查询，并把端点公布的新模型**只增**地写入该路由的 `models` 列表。已提供的模型会原样保留，因此用户调优过的条目不会丢失；没有新模型的路由保持不变。该开关默认关闭，因为该操作会在每次打开页面时通过线缆询问每个端点；选择结果保存在 `dsh.modelsSettings.autoSync` 中。
